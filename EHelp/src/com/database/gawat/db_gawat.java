...
public class db_gawat extends SQLiteOpenHelper {
	...
	/************************** CREATE TABLE **************************************/
	String TABLE_SEQ = "CREATE TABLE " + SEQ + "(" + seq_number
			+ " TEXT PRIMARY KEY  )";

	/************************** CREATE TABLE **************************************/
	String TABLE_SEQ_K = "CREATE TABLE " + SEQ_KELUHAN + "(" + seq_number_k
			+ " TEXT PRIMARY KEY  )";

	String CREATE_TABLE_DATADIRI = "CREATE TABLE " + data_diri + "(" + id_orang
			+ " TEXT PRIMARY KEY," + id_pembuat + " TEXT," + nama_orang
			+ " TEXT, " + tempat_lahir + " TEXT," + tanggal_lahir + " TEXT, "
			+ jenis_kelamin + " TEXT, " + golongan_darah + " TEXT," + no_telp
			+ " TEXT," + wa + " TEXT, " + bbm + " TEXT," + alamat + " TEXT, "
			+ penyakit_id + " TEXT," + foto + " TEXT," + waktu + " TEXT,"
			+ status + " TEXT" + ")";

	String CREATE_TABLE_KELUHAN = "CREATE TABLE " + keluhan + "(" + id_keluhan
			+ " TEXT PRIMARY KEY," + jenis_bantuan + " TEXT," + fk_id_korban
			+ " TEXT, " + id_pelapor + " TEXT," + fk_kode_rs + " TEXT, "
			+ nama_keluhan + " TEXT," + foto_keluhan + " TEXT," + longitude
			+ " TEXT, " + latitude + " TEXT," + status_keluhan + " TEXT, "
			+ jumlah_korban + " TEXT," + waktu_kejadian + " TEXT," + idpegawai
			+ " TEXT" + ")";

	String CREATE_TABLE_PENYAKIT = "CREATE TABLE " + penyakit + "("
			+ id_penyakit + " TEXT PRIMARY KEY," + nama_penyakit + " TEXT"
			+ ")";
	...
	String CREATE_TABLE_PETUGAS = "CREATE TABLE " + petugas + "(" + email
			+ " TEXT PRIMARY KEY," + password + " TEXT" + ")";

	String CREATE_TABLE_REGISTER = "CREATE TABLE " + register + "("
			+ namars_reg + " TEXT," + kode_reg + " TEXT," + id_pegawai
			+ " TEXT PRIMARY KEY," + rs_no_telp + " TEXT," + rs_alamat
			+ " TEXT," + jumlah_ambulance + " TEXT," + status_ambulance
			+ " TEXT" + ")";
	...
	public void onCreate(SQLiteDatabase db) {
		try {
			...
			Log.v(" Oncreate SQLException", Log.getStackTraceString(se));
		} catch (Exception e) {
			Log.v(" Oncreate Exception", Log.getStackTraceString(e));
		} finally {
			db.endTransaction();
		}
	...
	public Integer GetSeq() {
		...
		try {
			Cursor mCursor = db.rawQuery("SELECT seq_num FROM  SEQ ", null);
			if (mCursor != null) {
				mCursor.moveToFirst();
	...
	public Integer GetSeq_Keluhan() {
		...
		SQLiteDatabase db = this.getReadableDatabase();
		try {
			Cursor mCursor = db.rawQuery("SELECT seq_num_k FROM  SEQ_KELUHAN ",
					null);
			if (mCursor != null) {
				mCursor.moveToFirst();
				result = mCursor.getString(0);
	...
	/************************** data diri *****************************/
	public void InsertDataDiri(String id_org, String id_pbt, String nama_org,
			String tempat_lhr, String tgl_lhr, String jen_kel, String golda,
			String notelp, String wa_, String bbm_, String alamat_,
			String penyakit_id_, String foto_, String waktu_, String status_) {
		SQLiteDatabase db = this.getWritableDatabase();
		try {
			ContentValues values = new ContentValues();
			...
			Log.v("Insert tabel data diri",
					"id_org"
							...
							+ " nama orang"
							+ nama_org
							+ " tmpt lahir tgl lahir golda notelp bbm wa alamat penyakit_id foto waktu");
			db.close();
		} catch (SQLiteException se) {
...

	{
		...
		Cursor cursor = getReadableDatabase().rawQuery(
				"select * from data_diri where id_orang = '" + id + "'", null);
	...
	public ArrayList<DataDiri> GetAllData(String id) {
		...
		try {
			Cursor mCursor = db.rawQuery(
					"SELECT *  FROM  data_diri WHERE id_pembuat= '" + id + "'",
					null);
			if (mCursor != null) {
				mCursor.moveToFirst();
	...
	public void UpdateDataDiri(String id_org, String id_pbt, String nama_org,
			String tempat_lhr, String tgl_lhr, String jen_kel, String golda,
			String notelp, String wa_, String bbm_, String alamat_,
			String penyakit_id_, String foto_, String waktu_, String status_) {
		...
		values.put(status, status_);
		try {
			db.update(data_diri, values, "id_orang = ?",
					new String[] { id_org });
		} catch (SQLiteException se) {
	...
	public ArrayList<DataDiri> getSemuaDataPasien() {
		...
		cursor = null;
		try {
			cursor = db.query(data_diri, new String[] { id_orang, id_pembuat,
					nama_orang, tempat_lahir, tanggal_lahir, jenis_kelamin,
					golongan_darah, no_telp, wa, bbm, alamat, penyakit_id,
					foto, waktu, status }, null, null, null, null, null);
	...
	public DataDiri ViewDataDiri(String id) {
		...
		try {
			...
			String Query = "SELECT * FROM data_diri where id_orang = '" + id
					+ "'";
	...
	/*-----------------------------Create Keluhan ------------------------------*/
	public void InsertKeluhan(String id_keluhan_, String jenis_bantuan_,
			String fk_id_korban_, String id_pelapor_, String fk_kode_rs_,
			String nama_keluhan_, String foto_keluhan_, String longitude_,
			String latitude_, String status_keluhan_, String jumlah_korban_,
			String waktu_kejadian_, String id_pegawai_) {
		SQLiteDatabase db = this.getWritableDatabase();
		try {
			ContentValues values = new ContentValues();
	...
	/*-----------------------------Create Register ------------------------------*/
	public void InsertRegister(String namars_reg_, String kode_reg_,
			String id_pegawai_, String rs_no_telp_, String rs_alamat_,
			String jumlah_ambulance_, String status_ambulance_) {
		SQLiteDatabase db = this.getWritableDatabase();
		try {
			ContentValues values = new ContentValues();
			...
			// Log.v("Insert tabel data diri", "id_org"+id_org+
			// "id_pembuat"+id_pbt+" nama orang"+nama_org+" tmpt lahir tgl lahir golda notelp bbm wa alamat penyakit_id foto waktu");
			db.close();
		} catch (SQLiteException se) {
	...
	public ArrayList<RegisterModel> getSemuaDataRegister() {
		...
		cursor = null;
		try {
			cursor = db.query(register, new String[] { namars_reg, kode_reg,
					id_pegawai, rs_no_telp, rs_alamat, jumlah_ambulance,
	...
	// public void InsertDataRS(String nama_rs_, String kode_rs_, String
	// rs_no_telp_, String rs_email_,
	// String rs_alamat_, String rs_longitude_, String rs_latitude_, String
	// jumlah_ambulance_,
